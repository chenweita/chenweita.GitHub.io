...
如何提升页面响应交互？
- CPU卡顿：大量计算操作导致的性能问题，掉帧卡顿，滚动，输入，用户感觉不流畅
- IO卡顿：网络请求延时的，无法快速响应，下载解析慢

浏览器引擎：
- js引擎 v8/SpiderMonkey
- 渲染引擎 Blink/Gecko
...
yieldInterval：5ms执行自身的代码，相关计算工作
...
concurrent:
unstable_scheduleCallback(() =>  this.setState())
...
Q：
- JSX和Fiber节点是同一个东西么？ 
  jsx静态数据，手动写出来的，Fiber动态更新机制数据，react应用本身的一种更新机制
- React Component、React Element是同一个东西么，他们和JSX有什么关系？

JSX在编译时会被Babel编译为React.createElement方法，这也是为什么要引入import React from 'react';的原因
- React.createELement
...
window.performance可以达到微妙级，new Date()只能达到毫秒级
### 5.1.2 beginWork
...
子组件不为空时直接把指针给到子组件，否则对子组件调用completeUnitOfWork
...
- layout
主要包括三点内容：
1. useEffect相关的处理：beforemount执行
